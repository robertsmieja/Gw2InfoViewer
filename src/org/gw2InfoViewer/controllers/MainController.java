...
import org.gw2InfoViewer.services.json.JsonService;
...
//    private static final String WORLD_ID = "";
    ...
    protected MainController() {
        ...
        try {
            ...
            events = HttpsConnectionFactory.getStringFromHttpResponse(httpClient.execute(getEvents));
        ...
        this.eventList = jsonService.parseEventListJson(events);
        this.eventNameMap = jsonService.parseNameMapJson(eventNames);

        for (Event event : this.eventList.getEventList()) {
            event.setEventName(this.eventNameMap.getNameMap().get(event.getEventId()));
        }

        view.setEventsListModel(eventList);
        view.setEventNameMap(eventNameMap);
    }
